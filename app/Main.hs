...
import           AST
import           Statement.Lexing
import           Statement.Translator
...
functionExecutionArgParser :: Parser (Maybe [AExpr])
functionExecutionArgParser = optional $ sepBy1 aExpr sep
...
functionExecution = do
  fun <- identifier
  args <- fromMaybe Nothing <$> optional (parens functionExecutionArgParser)
  m <- optional more'
